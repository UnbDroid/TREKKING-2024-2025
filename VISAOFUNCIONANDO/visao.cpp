...
void sonda(Mat img){
    ...
    // waitKey(1);
	temp = img.clone();
	float ratio_original = (float)img.cols/(float)img.rows;    //imagem temporaria é so a imgNormal porem ajustada o tamanho   
    
    
	cvtColor(temp, hsv, cv::COLOR_BGR2HSV);
    // cv::imshow("Imagem temporaria",hsv);
