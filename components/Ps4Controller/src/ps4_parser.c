...
/********************/
ps4_sensor_t parsePacketSensor(uint8_t *packet) {
  ps4_sensor_t ps4Sensor = {0};
  /*
      const uint16_t offset = 0x200;

      ps4Sensor.accelerometer.x = (packet[packet_index_sensor_accelerometer_x]
     << 8) + packet[packet_index_sensor_accelerometer_x+1] - offset;
      ps4Sensor.accelerometer.y = (packet[packet_index_sensor_accelerometer_y]
     << 8) + packet[packet_index_sensor_accelerometer_y+1] - offset;
      ps4Sensor.accelerometer.z = (packet[packet_index_sensor_accelerometer_z]
     << 8) + packet[packet_index_sensor_accelerometer_z+1] - offset;
