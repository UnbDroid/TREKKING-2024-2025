...
static esp_vhci_host_callback_t vhci_host_cb = {controller_send_ready,
                                                host_rcv_pkt};

void btd_vhci_mutex_lock() { xSemaphoreTake(lock_mutex, portMAX_DELAY); }

void btd_vhci_mutex_unlock() { xSemaphoreGive(lock_mutex); }
...
esp_err_t btd_vhci_init(bool createTask) {
  ...
  if ((ret = esp_bt_controller_init(&bt_cfg)) != ESP_OK) {
    ESP_LOGE(LOG_TAG, "ESP Bluetooth controller initialize failed: %s",
             esp_err_to_name(ret));
    return ESP_FAIL;
  }

  if ((ret = esp_bt_controller_enable(ESP_BT_MODE_CLASSIC_BT)) != ESP_OK) {
    ESP_LOGE(LOG_TAG, "ESP Bluetooth controller enable failed: %s",
             esp_err_to_name(ret));
    return ESP_FAIL;
  }

  if ((ret = esp_vhci_host_register_callback(&vhci_host_cb)) != ESP_OK) {
    ESP_LOGE(LOG_TAG, "ESP Bluetooth callback register fail: %s",
             esp_err_to_name(ret));
    return ESP_FAIL;
  }
  ...
  if (createTask) {
    xTaskCreatePinnedToCore(btd_vhci_update_task, "btd_vhci_update_task",
                            10 * 1024, NULL, 2, NULL, 0);
...
void btd_vhci_autoconnect_task(void *task_params) {
  ...
  uint8_t saved_addr[6] = {37, 75, 75, 84, 20, 70};
  ...
  if (err == ESP_OK) {
    ESP_LOGI(LOG_TAG,
             "Read saved device address & connecting: %02X%02X%02X%02X%02X%02X",
             saved_addr[5], saved_addr[4], saved_addr[3], saved_addr[2],
             saved_addr[1], saved_addr[0]);
  ...
  if (!bthid->connected) {
    ESP_LOGI(LOG_TAG,
             "Could not connect to saved device, going to pairing mode");
    ...
    while (1) {
      if (bthid->connected) {
        ...
        ESP_LOGI(LOG_TAG, "Saving device address: %02X%02X%02X%02X%02X%02X",
                 saved_addr[5], saved_addr[4], saved_addr[3], saved_addr[2],
                 saved_addr[1], saved_addr[0]);
...
esp_err_t btd_vhci_autoconnect(BTHID *bthid) {
  ...
  xTaskCreatePinnedToCore(btd_vhci_autoconnect_task,
                          "btd_vhci_autoconnect_task", 10 * 1024, bthid, 2,
                          NULL, 1);
